...
export function AlbumScan({ dir, onOpen, onClose }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    const outlier = (r: ScanRow) =>
      rows.length > 2 && (Math.abs((r.cutoffHz ?? 22050) - med) > 700 || (r.level && r.level !== major))
          ...
          <tbody>
            {rows.map(r => {
              ...
              return (
                <tr
                  key={r.name}
                  onClick={() => !r.error && onOpen(base + r.name)}
                  tabIndex={0}
                  onKeyDown={e => {
                    if (e.key === "Enter" && !r.error) onOpen(base + r.name)
                  }}
                ...
                >
                  <td className="px-3 py-2">
                    <Icon
                      aria-hidden="true"
                      className={cn("size-4", r.level ? TONE[r.level] : "text-muted-foreground")}
                    />
                  </td>
                  ...
                  <td className={cn("px-3 py-2 font-mono", (r.clipEvents ?? 0) > 0 && "text-warning")}>
                    {r.clipEvents ?? "-"}
                  </td>
