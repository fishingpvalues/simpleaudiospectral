...
function recent(): string[] {
  try { return JSON.parse(localStorage.getItem("spectrals.recent") ?? "[]") } catch { return [] }
}
...
export function Library({ dir, setDir, current, onOpen, onScan }: Props) {
  ...
  useEffect(() => {
    let live = true
    setQuery(""); setHits(null); setSel(0)
    api.ls(dir).then(l => { if (live) { setListing(l); setError(null) } }).catch(e => live && setError(String(e.message ?? e)))
    return () => { live = false }
  }, [dir])

  useEffect(() => {
    api.roots().then(r => { setRoots(r.roots); setIndexed({ ready: r.indexed, entries: r.entries }) }).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    setSearching(true)
    const t = setTimeout(() => {
      api.search(query, ctl.signal).then(r => { setHits(r.results); setIndexed(i => ({ entries: i?.entries ?? 0, ready: r.ready })) })
  ...
  const items: Item[] = useMemo(() => {
    ...
    for (const d of listing?.dirs ?? []) if (!shown.has(d.name) && (!f || d.name.toLowerCase().includes(f))) out.push({ kind: "dir", key: "d:" + d.name, path: join(d.name), name: d.name })
    for (const x of listing?.files ?? []) if (!f || x.name.toLowerCase().includes(f)) out.push({ kind: "file", key: "f:" + x.name, path: join(x.name), name: x.name, size: x.size })
    return out
  ...
  const onKey = (e: React.KeyboardEvent) => {
    ...
    else if (e.key === "Enter" && items[sel]) { e.preventDefault(); activate(items[sel]) }
    else if (e.key === "Escape") { setQuery(""); (e.target as HTMLElement).blur() }
    else if (e.key === "Backspace" && !query && dir && e.target === inputRef.current) { e.preventDefault(); setDir(dir.split("/").slice(0, -1).join("/")) }
  }

  // "/" focuses the search from anywhere, like most file browsers.
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (e.key === "/" && !(e.target as HTMLElement).closest("input,textarea")) { e.preventDefault(); inputRef.current?.focus() }
    }
    ...
    <div className="flex h-full min-h-0 flex-col" onKeyDown={onKey}>
      <div className="flex items-center gap-1 px-3 pt-3 pb-2">
        <nav className="flex min-w-0 flex-1 flex-wrap items-center gap-0.5 text-xs text-muted-foreground">
          <button className="hover:text-foreground" onClick={() => setDir("")}>library</button>
          {parts.map((p, i) => (
            <span key={i} className="flex min-w-0 items-center gap-0.5">
              <ChevronRight className="size-3 shrink-0" />
              <button className="max-w-36 truncate hover:text-foreground" title={p} onClick={() => setDir(parts.slice(0, i + 1).join("/"))}>{p}</button>
            </span>
          ))}
        </nav>
        {audioHere && (
          <Tip label="Scan every track in this folder">
            <Button variant="outline" size="sm" onClick={() => onScan(dir)}><ListChecks />Scan</Button>
          </Tip>
        )}
      </div>
      <div className="relative px-3 pb-2">
        ...
        <Input ref={inputRef} value={query} onChange={e => { setQuery(e.target.value); setSel(0) }}
          placeholder="Search library" className="pr-14 pl-8" aria-label="Search library" />
        <div className="absolute top-1.5 right-5 flex items-center gap-1">
          ...
          {query ? (
            <button onClick={() => setQuery("")} aria-label="Clear search" className="text-muted-foreground hover:text-foreground"><X className="size-4" /></button>
          ) : <Kbd>/</Kbd>}
        </div>
      </div>
      {hits && (
        <div className="px-4 pb-1 text-[11px] text-muted-foreground">
          {hits.length} {hits.length === 1 ? "result" : "results"} across the library{indexed && !indexed.ready ? " (index still building)" : ""}
        </div>
      )}
      <ScrollArea className="min-h-0 flex-1">
        <div ref={listRef} className="px-1.5 pb-3" role="listbox" aria-label="Library">
          {!dir && !hits && roots.length > 0 && (
            <Section label="Volumes">
              {roots.map(r => {
                const used = r.total && r.free != null ? 1 - r.free / r.total : null
                return (
                  <button key={r.name} onClick={() => setDir(r.name)}
                    className="mb-1 w-full rounded-md border px-2.5 py-2 text-left hover:bg-accent">
                    <div className="flex items-center gap-2 text-sm"><HardDrive className="size-4 text-muted-foreground" /><span className="truncate">{r.name}</span></div>
                    {used != null && r.total && (
                      <>
                        <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-muted"><div className="h-full bg-foreground/70" style={{ width: `${used * 100}%` }} /></div>
                        <div className="mt-1 text-[11px] text-muted-foreground">{fmtBytes(r.free ?? 0)} free of {fmtBytes(r.total)}</div>
                      </>
                    )}
                  </button>
                )
          ...
          {rec.length > 0 && (
            <Section label="Recent">
              {rec.map(p => (
                <Row key={"r:" + p} icon={<Clock />} label={p.split("/").pop() ?? p} sub={p.split("/").slice(0, -1).join("/")}
                  active={current === p} onClick={() => onOpen(p)} />
              ))}
            </Section>
          )}
          {!dir && !hits && roots.length > 0 && items.length > 0 && <div className="px-2 pt-2 pb-1 text-[11px] font-medium tracking-wide text-muted-foreground uppercase">Folders</div>}
          {items.map((it, i) => (
            <Row key={it.key} idx={i} focused={i === sel}
          ...
          {listing && !items.length && <p className="px-2 py-4 text-sm text-muted-foreground">{hits ? "No matches." : "Nothing here."}</p>}
        </div>
      </ScrollArea>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-2">
      <div className="px-2 pt-2 pb-1 text-[11px] font-medium tracking-wide text-muted-foreground uppercase">{label}</div>
      {children}
    </div>
  )
}

function Row({ icon, label, sub, meta, onClick, active, focused, idx }: {
  icon: React.ReactNode; label: string; sub?: string; meta?: string; onClick: () => void; active?: boolean; focused?: boolean; idx?: number
}) {
  return (
    <button
      data-idx={idx}
