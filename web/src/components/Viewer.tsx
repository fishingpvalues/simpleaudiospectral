...
export function Viewer({
  ...
  const clampView = useCallback(
    (v: View): View => {
      let { t0, t1, f0, f1 } = v
      const span = Math.min(dur, Math.max(MIN_SPAN_T, t1 - t0))
      if (t0 < 0) t0 = 0
      if (t0 + span > dur) t0 = Math.max(0, dur - span)
      t1 = t0 + span
      const fs = Math.min(nyq, Math.max(MIN_SPAN_F, f1 - f0))
      if (f0 < 0) f0 = 0
  ...
  const onViewSpectrumRef = useRef(onViewSpectrum)
  useEffect(() => {
    onViewSpectrumRef.current = onViewSpectrum
  }, [onViewSpectrum])
  const derive = useCallback((s: Stft) => {
    ...
    rolloff.current = ro
    if (onViewSpectrumRef.current) {
      const outHz = new Float32Array(rows),
        outDb = new Float32Array(rows)
      for (let r = 0; r < rows; r++) {
        outHz[rows - 1 - r] = hz[r]
        outDb[rows - 1 - r] = 10 * Math.log10(mean[r] / cols + 1e-30)
      }
      onViewSpectrumRef.current({ hz: outHz, db: outDb })
    }
  }, [])
        ...
        const s = await api.stft(
          {
            ...
            cols: Math.min(4096, Math.round(plotW * dpr)),
            rows: Math.min(2048, Math.round(specH * dpr)),
          },
          ctl.signal,
        )
        stft.current = s
        ...
        const data = await api.wave(
          {
            ...
            t0: view.t0.toFixed(4),
            t1: view.t1.toFixed(4),
            cols: Math.min(8192, Math.round(plotW * dpr)),
          },
          ctl.signal,
        )
  ...
  useEffect(() => {
    ...
    const ctl = new AbortController()
    api
      .wave({ path: info.path, ch: "mix", t0: 0, t1: dur, cols: Math.min(4096, plotW) }, ctl.signal)
      .then(d => {
        overview.current = d
        redraw()
      })
  ...
  // ------------------------------------------------------------- colouring
  const table = useMemo(
    () => lut(settings.cmap, settings.floor, settings.ceil, -160, 0),
    [settings.cmap, settings.floor, settings.ceil],
  )
  useEffect(() => {
    colored.current = null
    redraw()
  }, [table, redraw, settings.holes])
  ...
  useEffect(() => {
    ...
    if (!g) return
    g.fillStyle = "#0a0a0a"
    g.fillRect(0, 0, plotW, RULER_H)
    const { step, ticks } = timeTicks(view.t0, view.t1, plotW)
    g.font = "11px 'Geist Mono Variable', monospace"
    g.textBaseline = "top"
    for (const t of ticks) {
      const x = Math.round(xOf(t)) + 0.5
      g.strokeStyle = "rgba(255,255,255,0.35)"
      g.beginPath()
      g.moveTo(x, RULER_H - 7)
      g.lineTo(x, RULER_H)
      g.stroke()
      g.fillStyle = "#a3a3a3"
  ...
  useEffect(() => {
    ...
    if (!g) return
    g.fillStyle = "#050505"
    g.fillRect(0, 0, plotW, OVERVIEW_H)
    ...
    if (d) {
      ...
      g.fillStyle = "#525252"
      for (let i = 0; i < n; i++) {
        const y1 = mid - d[2 * i + 1] * amp,
          y2 = mid - d[2 * i] * amp
        g.fillRect((i / n) * plotW, y1, Math.max(1, plotW / n), Math.max(1, y2 - y1))
      }
    }
    const x0 = (view.t0 / dur) * plotW,
      x1 = (view.t1 / dur) * plotW
    g.fillStyle = "rgba(255,255,255,0.08)"
    g.fillRect(x0, 0, Math.max(2, x1 - x0), OVERVIEW_H)
  ...
  const drawOverlay = useCallback(() => {
    ...
    if (settings.grid) {
      ...
      g.lineWidth = 1
      g.beginPath()
      for (const f of freqTicks(view, settings.scale, specH)) {
        const y = Math.round(yOf(f)) + 0.5
        g.moveTo(0, y)
        g.lineTo(plotW, y)
      }
    ...
    if (settings.cutoff && a?.cutoffHz && a.cutoffHz >= view.f0 && a.cutoffHz <= view.f1) {
      ...
      if (a.shelf16k && 16000 >= view.f0 && 16000 <= view.f1) {
        ...
        g.fillStyle = "rgba(0,0,0,0.6)"
        g.fillRect(plotW - w2 - 16, ys - 20, w2 + 10, 18)
        g.fillStyle = "#fff"
        g.fillText(l2, plotW - w2 - 11, ys - 4)
      }
    }
    const ro = rolloff.current,
      sm = stft.current
    if (settings.rolloff && ro && sm && sm.meta.scale === settings.scale) {
      const m = sm.meta
      g.strokeStyle = overlayColor(settings.cmap).css
      g.lineWidth = 1.25
      g.beginPath()
      let pen = false
    ...
    if (m && !d) {
      ...
      g.lineTo(Math.round(m.x) + 0.5, specH)
      if (m.area === "spec") {
        g.moveTo(0, Math.round(m.y) + 0.5)
        g.lineTo(plotW, Math.round(m.y) + 0.5)
      }
      g.stroke()
    }
    if (audio) {
  ...
  function drawWavePlayhead() {
    ...
    if (d?.kind === "wave") {
      ...
      g.fillRect(x, 0, Math.abs(d.x1 - d.x0), WAVE_H)
    }
    const m = mouse.current
    if (m && !d) {
      g.strokeStyle = "rgba(255,255,255,0.45)"
      g.beginPath()
      g.moveTo(Math.round(m.x) + 0.5, 0)
      g.lineTo(Math.round(m.x) + 0.5, WAVE_H)
      g.stroke()
    }
    if (audio) {
      const x = xOf(audio.currentTime)
  ...
  useEffect(() => {
    ...
    registerExport(() => {
      ...
      const a = info.analysis
      g.fillText(
        `${info.codecName?.toUpperCase()} ${(info.sampleRate / 1000).toFixed(1)} kHz ${info.bits ?? "?"} bit  |  ${fmtTime(view.t0, 0.01)}-${fmtTime(view.t1, 0.01)}  ${fmtHz(view.f0)}-${fmtHz(view.f1)} Hz  |  ${settings.ch} ${settings.scale} FFT ${settings.fft} ${settings.win}  |  cut-off ${a?.cutoffHz ? (a.cutoffHz / 1000).toFixed(2) + " kHz" : "none"}`,
        10,
        26,
      )
  ...
  return (
    ...
    >
      <canvas
        ...
        aria-hidden="true"
        onPointerDown={e => {
          if (audio) audio.currentTime = tOf(local(e, e.currentTarget).x)
        }}
      />
